...
class CheXpert(object):
    ...
    def __init__(self, root ='/home/mjkim1/.cache/kagglehub/datasets/ashery/chexpert/versions', num_clients=20 ,alpha=1):
        # self.dataset_dir = os.path.join(root, self.dataset_dir)
        self.dataset_dir = os.path.join(root,'1')
        self.root= root 
    ...
    def get_non_iid_data_train_test_val(self, dataset_dir, num_clients, test_ratio=0.1,alpha=0.3):
        train_file = os.path.join(dataset_dir, "train_only_frontal.csv")
        ...
        with open(os.path.join('datasets/chexpert_partition_{}_al_frontal.json'.format(alpha))) as json_file:
    ...
    def get_data_loaders (self, batch_size = 64, test_batch_size =64, server_batch_size=128,img_resolution=None,kd_data_fraction=1):
        ...
        image_size = 256 if img_resolution is None else img_resolution
        ...
        for user_id in range(num_clients):  
            train_data.update({user_id: {'dataloader':  torch.utils.data.DataLoader(DatasetWrapper(self.local_train_list[user_id],train_transform), batch_size= batch_size, num_workers=16, pin_memory=True,
                                shuffle =True,persistent_workers=True), 
                            'indices': self.local_train_list[user_id]  }})

            test_data.update({user_id: {'dataloader':  torch.utils.data.DataLoader(DatasetWrapper(self.local_test_list[user_id],test_transform), batch_size= test_batch_size, num_workers=16, pin_memory=True,
                            shuffle =True,persistent_workers=True), 
