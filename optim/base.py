...
class FedBase:
    ...
    def train(self, model, trainloader, epoch, optimizer, log=False, u=0):
        ...
        model.zero_grad()
        model.train()
        # print(f"client {u} loss is {criterion}")
        ...
        if self.args.dataset == "chexpert":
            ...
            with torch.no_grad():
                ...
                if log:
                    ...
                    print(
                        f"test AL Loss: {(adv_loss / (batch_idx +1)):.3f} | AL Acc: {100.0 * adv_correct / total}"
                    )

            acc = 100.0 * correct / total
            return adv_loss / len(testloader), acc
