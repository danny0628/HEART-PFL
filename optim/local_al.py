...
class FedBaseAL:
    ...
    def __init__(
        ...
        self.clients_losses = clients_losses
    ...
    def run_one_round(self, current_round):
        ...
        ## 여기서 업데이트된 local model이 서버로 넘어가서 fedavg+KD
        ## 이렇게 서버에서 업데이트된 local model은 self.global_model에 저장됨
    ...
    def train(self, model, trainloader, epoch, optimizer, log=False, u=0):
        ...
        # criterion = self.clients_losses[u]
    ...
    def train_al(self, model, trainloader, epoch, optimizer, log=False, u=0):
        ...
        model.zero_grad()
        model.train()
        adversary = torchattacks.PGD(al_net)
        
        def train_core_al(data_loader, model, epoch, optimizer):
            for ep in range(epoch):
                train_loss = 0
                correct = 0
                total = 0
        ...
        _loss, _acc = train_core_al(trainloader, model, epoch, optimizer)
