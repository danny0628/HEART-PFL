...
class AL(FedBaseAL):
    ...
    def server_distillation(
        ...
        from aggregation.distillation import Distillation, DGAD, Distillation_origin, Distillation_mapping
    ...
    def run_local_updates(
        ...
        u_train_per_loss, u_train_per_acc, u_d_k = self.train_per(
            per_model,
            local_model,
            self.args.lmbda,
            train_dataloader_u,
        ...
        def train_core(data_loader, model, epoch, optimizer):
            for ep in range(epoch):
                ...
                for batch_idx, (x, y) in enumerate(data_loader):
                    ...
                    '''
                    FedAS : intra aglinment implement
                    
                    # for i in range(len(y)):
                    #     label = y[i].item()
                    #     if label in mean_prototypes:
                    #         align_loss += F.mse_loss(global_features[i], mean_prototypes[label])
                    # align_loss /= len(y)
