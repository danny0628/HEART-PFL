...
class AL2(FedBaseAL):
    ...
    def server_distillation(
        ...
        from aggregation.distillation import Distillation, DGAD, Distillation_origin

        distill_model, distill_step = Distillation_origin(
            self.args,
    ...
    def train_per(
        ...
        def train_core(data_loader, model, epoch, optimizer):
            for ep in range(epoch):
                ...
                # print(
                #     f"ce loss : {test1/(batch_idx + 1)}, geo loss : {test2/(batch_idx + 1)}"
                # )
